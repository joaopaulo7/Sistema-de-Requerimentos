...
class ListagemModel extends CI_Model { 
    ...
    public function getListVis($funcao) {
		...
		$this->db->where( $funcao." =".$this->session->userdata('login'));
    ...
    public function getListSubs(){
        ...
        $this->db->where("professor_substituto =".$this->session->userdata('login'));
		$this->db->or_where("coordenador =".$this->session->userdata('login'));
